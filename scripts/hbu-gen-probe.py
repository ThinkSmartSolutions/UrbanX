#!/usr/bin/env python3
# Generează HBU pe parcelă (Lăpușneanu, Iași) headless, salvează PDF + inspectează datele reale.
...
print("astept _HBU + map..."); ok=False
for _ in range(60):
    ok=ev("!!(window._HBU&&window._HBU.generatePDF&&(window.jspdf||window.jsPDF))")
    if ok is True: break
    time.sleep(1)
print("  ready:",ok)
...
print("  generez HBU (asteapta fetch RAN/OSM/DEM)...")
res=ev("(async()=>{ try{ await window._HBU.generatePDF('RO-IS-01'); return 'done'; }catch(e){ return 'ERR '+e.message; } })()")
