...
for i in range(120):
    ...
    if sid=='b17s1':
        seen_street=True
        b = ev("window.__M && +window.__M.getBearing().toFixed(0)")
        ctr = ev("(function(){var c=window.__M&&window.__M.getCenter();return c?[+c.lng.toFixed(4),+c.lat.toFixed(4)]:null})()")
        print(f"  [STREET] t={street_t*0.5:4.1f}s  zoom={z}  pitch={p}  bearing={b}  center={ctr}")
        street_t+=1
        if street_t>40: break
    else:
