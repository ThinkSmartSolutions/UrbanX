  ...
  async function generatePDF(cityKey, type) {
    ...
    try {
      ...
      D.bullets && D.bullets([
        ...
        'Captarea plusvalorii (Land Value Capture) — contribuție la aprobarea PUZ cu majorare de CUT, întrucât reglementarea creează plusvaloare privată din decizie publică (Legea 169/2026 (CATUC), mecanism voluntar de mediere);',
      ...
      D.chapter && D.chapter('10. Context de piață și factori macroeconomici');
      D.P && D.P('Valoarea imobiliară nu este o mărime statică, ci rezultatul echilibrului dinamic dintre cerere și ofertă, modulat de condițiile macroeconomice. Pe partea cererii acționează: veniturile gospodăriilor și accesibilitatea creditului (nivelul dobânzilor ipotecare este, de regulă, factorul cu cel mai mare impact pe termen scurt), dinamica demografică (migrația către orașele-magnet, formarea de noi gospodării), precum și cererea investițională (imobilul ca rezervă de valoare în perioade inflaționiste). Pe partea ofertei contează stocul existent, ritmul autorizărilor și costurile de construcție (materiale, manoperă, finanțare).');
      D.P && D.P('Pentru piața românească, ultimii ani au fost marcați de o cerere robustă susținută de creșterea veniturilor și de programe de creditare, întâlnind o ofertă constrânsă de costurile de construcție și de durata autorizării. Rezultatul a fost o apreciere nominală susținută, mai accentuată în orașele-hub. Riscurile principale pentru perioada următoare sunt creșterea/menținerea dobânzilor la niveluri ridicate (care reduce puterea de cumpărare) și o eventuală încetinire economică; ambele ar tempera ritmul de apreciere, fără a inversa, în general, ierarhia spațială a valorii descrisă în acest studiu.');
      D.P && D.P('Inflația joacă un rol dublu: pe de o parte erodează valoarea reală a chiriilor fixe, pe de alta împinge investitorii către active reale precum imobilele, susținând prețurile nominale. Pentru o interpretare corectă, valorile din acest studiu trebuie citite în termeni nominali, la momentul calibrării, și actualizate periodic cu evoluția indicelui de preț al pieței.');

      D.chapter && D.chapter('11. Metode de evaluare imobiliară');
      D.P && D.P('Standardele de evaluare (IVS — International Valuation Standards, adoptate și de ANEVAR) recunosc trei abordări fundamentale, complementare. Abordarea prin COMPARAȚIE (market approach) estimează valoarea pe baza tranzacțiilor recente cu proprietăți similare, ajustate pentru diferențe — este metoda dominantă pentru locuințe și terenuri pe piețe active. Abordarea prin VENIT (income approach) capitalizează veniturile nete generate de proprietate (chirii) la o rată de randament — relevantă pentru imobile comerciale și de investiții. Abordarea prin COST estimează costul de reconstrucție minus deprecierea plus valoarea terenului — utilă pentru proprietăți specializate, fără piață comparabilă.');
      D.P && D.P('Modelul de rentă urbană din prezentul studiu nu înlocuiește aceste metode, ci le precede și le contextualizează: el oferă o hartă a nivelului general de valoare pe zone (un „câmp" de referință), în interiorul căruia o evaluare individuală prin comparație sau venit se poate ancora. Practic, studiul răspunde la întrebarea „cât valorează, în medie, un metru pătrat în această zonă?", în timp ce evaluarea ANEVAR răspunde la „cât valorează exact această proprietate?". Cele două sunt complementare, nu concurente.');

      D.chapter && D.chapter('12. Impactul reglementării urbanistice asupra valorii');
      D.P && D.P('Reglementarea urbanistică (PUG/PUZ) este unul dintre cei mai puternici determinanți ai valorii terenului, prin coeficienții POT (procent de ocupare) și CUT (coeficient de utilizare — suprafața desfășurată maximă admisă pe unitatea de teren). Un CUT mai mare permite o suprafață construibilă mai mare pe același teren, crescând direct valoarea reziduală a terenului (metoda reziduală: valoare teren = valoarea dezvoltării finalizate − costuri de construcție − profit dezvoltator). De aceea, o majorare de CUT prin PUZ transferă, în esență, valoare publică (dreptul de a construi mai mult) către proprietarul privat.');
      D.P && D.P('Acest mecanism justifică instrumentele de captare a plusvalorii (Land Value Capture): atunci când decizia publică creează plusvaloare privată, o parte din aceasta poate fi recuperată pentru comunitate (infrastructură, spații verzi, locuințe accesibile). În România, Legea 169/2026 (CATUC) (art. 56) permite negocierea unor contribuții în cadrul acordului la aprobarea PUZ. Harta de valoare cuantifică tocmai această plusvaloare, oferind administrației o bază obiectivă pentru negociere și pentru evitarea subevaluării contribuției.');

      D.chapter && D.chapter('13. Grila notarială, fiscalitate și echitate');
      D.P && D.P('Grila notarială (expertizele de valori minime imobiliare, actualizate periodic de camerele notarilor) servește ca bază de calcul pentru taxele de tranzacționare și pentru impozitare. În practică, grila se actualizează cu întârziere și are valori conservatoare (sub piață), pentru a nu împovăra tranzacțiile — ceea ce creează un decalaj tipic de 20–60% față de prețurile reale. Acest decalaj are consecințe fiscale: venituri sub potențial din taxe de tranzacționare și un impozit pe proprietate care nu reflectă valoarea reală.');
      D.P && D.P('Harta de valoare oferă administrației locale instrumentul pentru o fiscalitate mai echitabilă și mai eficientă: actualizarea periodică a grilei pe baze obiective, diferențierea impozitului pe zone (astfel încât proprietățile din zonele scumpe să contribuie proporțional cu beneficiul de localizare) și identificarea zonelor subevaluate fiscal. Echitatea fiscală — principiul ca sarcina să fie proporțională cu capacitatea contributivă și cu beneficiul — este astfel servită direct, transformând harta dintr-un instrument de analiză într-unul de politică publică.');

      D.chapter && D.chapter('14. Concluzii și recomandări');
      D.P && D.P('Studiul cartografiază distribuția spațială a valorii imobiliare în ' + uat + ' (mediană ' + N(vmed) + ' €/mp, interval ' + N(vmin) + '–' + N(vmax) + ' €/mp), confirmând gradientul de rentă urbană centru–periferie modulat de funcțiune și accesibilitate. Rezultatul are caracter orientativ, dar oferă o bază obiectivă, transparentă și actualizabilă pentru decizii de politică publică și de investiții.');
      D.P && D.P('Recomandări pentru administrația locală: (1) utilizarea hărții pentru actualizarea periodică a grilei notariale/fiscale pe baze obiective; (2) diferențierea impozitului pe zone de valoare, pentru echitate fiscală; (3) instituirea unui mecanism de captare a plusvalorii la majorările de CUT prin PUZ; (4) corelarea investițiilor publice în infrastructură cu zonele de cel mai mare potențial de apreciere. Recomandări pentru actorii privați: ancorarea analizelor de fezabilitate pe valorile zonale, cu verificare prin evaluare ANEVAR înainte de tranzacții.');
      D.P && D.P('Studiul urmează a fi rafinat cu tranzacții reale (ANCPI) și cu variabile hedonice, evoluând de la un model de rentă urbană către un model de evaluare de masă calibrat — instrumentul standard al administrațiilor fiscale moderne.');

      try { if (window.UrbanXIVU && window.UrbanXIVU.renderSection) window.UrbanXIVU.renderSection(D); } catch (e) {}
      D.chapter && D.chapter('15. Limitări și disclaimer');
      ...
      var fn = ('Studiu_Valori_' + (window._asciiFile ? window._asciiFile(uat) : uat) + '_' + new Date().toISOString().slice(0, 10) + '.pdf').replace(/[ăĂâÂîÎșȘşŞțȚţŢ]/g,function(c){return {'ă':'a','Ă':'A','â':'a','Â':'A','î':'i','Î':'I','ș':'s','Ș':'S','ş':'s','Ş':'S','ț':'t','Ț':'T','ţ':'t','Ţ':'T'}[c]||c;}).replace(/[^a-zA-Z0-9._-]/g,'_');
