...
//   • studiu de AMPLASAMENT (UAT/punctual, 60+ pag)  — cheie _PROFILE_DEEP['<id>_inf']
//   • studiu TERITORIAL (strategic, 100+ pag) — cheie _PROFILE_DEEP['<id>_sup']
//   (clasificarea rang inferior/superior e INTERNĂ — nu apare niciodată în output)
...
(function (G) {
  ...
  var META = {
    litoral: { ac: [14, 116, 165], ico: '🌊',
      inf: { t: 'STUDIU INTEGRAT AL ZONEI COSTIERE — AMPLASAMENT LITORAL', badge: 'PROFIL LITORAL · STUDIU DE AMPLASAMENT',
      ...
      sup: { t: 'MASTERPLAN LITORAL ȘI GESTIUNEA INTEGRATĂ A ZONEI COSTIERE', badge: 'PROFIL LITORAL · STUDIU TERITORIAL / STRATEGIC',
    ...
    delta: { ac: [22, 137, 90], ico: '🦢',
      inf: { t: 'STUDIU DE AMPLASAMENT ÎN ARIE NATURALĂ PROTEJATĂ (DELTĂ/ZONE UMEDE)', badge: 'PROFIL DELTĂ · STUDIU DE AMPLASAMENT',
      ...
      sup: { t: 'STRATEGIE TERITORIALĂ PENTRU DELTĂ ȘI ZONE UMEDE — PLAN DE PEISAJ ȘI BIODIVERSITATE', badge: 'PROFIL DELTĂ · STUDIU TERITORIAL / STRATEGIC',
    ...
    baraj: { ac: [37, 99, 168], ico: '🏞',
      inf: { t: 'STUDIU DE AMPLASAMENT ÎN ZONA DE INFLUENȚĂ A UNUI BARAJ / LAC DE ACUMULARE', badge: 'PROFIL BARAJ · STUDIU DE AMPLASAMENT',
      ...
      sup: { t: 'STRATEGIE TERITORIALĂ HIDROENERGETICĂ ȘI DE GOSPODĂRIRE A BAZINULUI', badge: 'PROFIL BARAJ · STUDIU TERITORIAL / STRATEGIC',
    ...
    minier: { ac: [120, 86, 40], ico: '⛏',
      inf: { t: 'STUDIU DE AMPLASAMENT ÎN ZONĂ MINIERĂ / MONO-INDUSTRIALĂ', badge: 'PROFIL MINIER · STUDIU DE AMPLASAMENT',
      ...
      sup: { t: 'STRATEGIE DE RECONVERSIE ȘI REGENERARE A TERITORIULUI MINIER', badge: 'PROFIL MINIER · STUDIU TERITORIAL / STRATEGIC',
    ...
    salin: { ac: [120, 120, 160], ico: '🧂',
      inf: { t: 'STUDIU DE AMPLASAMENT ÎN ZONĂ SALINĂ / TURISM BALNEAR', badge: 'PROFIL SALIN · STUDIU DE AMPLASAMENT',
      ...
      sup: { t: 'MASTERPLAN DE TURISM BALNEAR ȘI VALORIFICAREA RESURSELOR SALINE', badge: 'PROFIL SALIN · STUDIU TERITORIAL / STRATEGIC',
    ...
    portuar: { ac: [30, 90, 120], ico: '⚓',
      inf: { t: 'STUDIU DE AMPLASAMENT ÎN ZONĂ PORTUARĂ (MARITIM/FLUVIAL)', badge: 'PROFIL PORTUAR · STUDIU DE AMPLASAMENT',
      ...
      sup: { t: 'MASTERPLAN PORTUAR ȘI STRATEGIE LOGISTICĂ MULTIMODALĂ REGIONALĂ', badge: 'PROFIL PORTUAR · STUDIU TERITORIAL / STRATEGIC',
    ...
    termal: { ac: [180, 90, 60], ico: '♨',
      inf: { t: 'STUDIU DE AMPLASAMENT CU RESURSĂ GEOTERMALĂ / BALNEARĂ', badge: 'PROFIL TERMAL · STUDIU DE AMPLASAMENT',
      ...
      sup: { t: 'MASTERPLAN DE STAȚIUNE BALNEARĂ ȘI VALORIFICARE GEOTERMALĂ', badge: 'PROFIL TERMAL · STUDIU TERITORIAL / STRATEGIC',
    ...
    silvic: { ac: [34, 120, 60], ico: '🌲',
      inf: { t: 'STUDIU DE AMPLASAMENT ÎN ZONĂ SILVICĂ / FORESTIERĂ', badge: 'PROFIL SILVIC · STUDIU DE AMPLASAMENT',
      ...
      sup: { t: 'STRATEGIE FORESTIERĂ REGIONALĂ ȘI CORIDOARE ECOLOGICE', badge: 'PROFIL SILVIC · STUDIU TERITORIAL / STRATEGIC',
    ...
    seismic: { ac: [200, 110, 40], ico: '🟠',
      inf: { t: 'STUDIU DE VULNERABILITATE SEISMICĂ A AMPLASAMENTULUI (ZONA VRANCEA)', badge: 'PROFIL SEISMIC · STUDIU DE AMPLASAMENT',
      ...
      sup: { t: 'STRATEGIE TERITORIALĂ DE REDUCERE A RISCULUI SEISMIC', badge: 'PROFIL SEISMIC · STUDIU TERITORIAL / STRATEGIC',
  ...
  async function generate(profileId, rank, cityKey) {
    ...
    // GATING: studiul DE AMPLASAMENT (inf) necesită parcelă + volum 3D (POT/CUT/suprafețe/capturi din proiect).
    if (rank === 'inf' && G._requireParcel3D && !G._requireParcel3D()) return;
    ...
    G.ss && G.ss((rank === 'sup' ? '📘 Generez studiu teritorial...' : '📗 Generez studiu de amplasament...') + ' · ' + (prof.label || profileId));
    try {
      var pdf = new J({ orientation: 'portrait', unit: 'mm', format: 'a4' });
      var D = G._makeStratDoc(pdf, { docTitle: (prof.label || 'PROFIL') + (rank === 'sup' ? ' · TERITORIAL' : ' · AMPLASAMENT'), cityName: cityName, accent: ac });
      ...
      D.callout && D.callout('Delimitarea studiului', (rank === 'sup'
        ? 'Document TERITORIAL / STRATEGIC: analizează zona și sistemul la scară regională, nu parcela. Componenta punctuală de amplasament se tratează în studiul de amplasament dedicat acestui profil — documente diferite, complementare.'
        : 'Document DE AMPLASAMENT: analizează punctual localitatea și amplasamentul vizat. Strategia teritorială la scară regională se tratează în studiul teritorial dedicat acestui profil — documente diferite, complementare.'));
      ...
      D.P('Studiul oferă o bază de fundamentare pentru deciziile de planificare specifice profilului „' + (prof.label || profileId) + '". Recomandările trebuie integrate în documentele strategice ale UAT (SIDU, PUG, planuri sectoriale) și în documentele teritoriale de nivel județean/național (PATJ/PATN), corelate cu sursele de finanțare disponibile (POR, PNRR, JTF, Interreg, buget local).');
      ...
      var _scope = (rank === 'sup') ? 'Teritorial' : 'Amplasament';
      var fn = (G._stratFileName ? G._stratFileName('Profil_' + profileId + '_' + _scope, { territorial: rank === 'sup', localitate: cityName })
        : ('Profil_' + profileId + '_' + _scope + '_' + cityName.replace(/[^\w]+/g, '_') + '.pdf'));
  ...
  // ── GATE comun: studiile DE AMPLASAMENT necesită parcelă selectată + volum 3D ──
  // (cerere Florin: nu generezi un studiu de amplasament fără parcelă + pre-proiectare).
  if (!G._requireParcel3D) {
    G._requireParcel3D = function () {
      var S = window.S || {};
      var ap = S.parcels && S.parcels[(S.activeParcel == null ? 0 : S.activeParcel)];
      if (!ap || !ap.geo || !ap.geo.geometry) { G.ss && G.ss('⚠️ Selectează mai întâi o parcelă pe hartă — studiile de amplasament sunt punctuale.'); return false; }
      if (!(S.vol && S.vol.genDone)) { G.ss && G.ss('⚠️ Generează mai întâi volumul 3D (pre-proiectare). Studiul de amplasament folosește POT/CUT, suprafețe și capturi din proiect.'); return false; }
      return true;
    };
    window._requireParcel3D = G._requireParcel3D;
  }

  // ── Meniu RAPOARTE: profilurile detectate, DOAR studiul DE AMPLASAMENT (parcelă) ──
  // Studiul TERITORIAL al profilului stă în meniul Teritoriu (openTerritorialPicker), nu aici.
  function renderMenu() {
    try {
      ...
      det = det.filter(function (d) { return META[d.id] && G._PROFILE_DEEP && G._PROFILE_DEEP[d.id + '_inf']; });
      ...
      host.innerHTML = det.map(function (d) {
        ...
        return '<div style="display:flex;align-items:center;gap:6px;padding:3px 4px;border-radius:6px" onmouseover="this.style.background=\'rgba(255,255,255,.05)\'" onmouseout="this.style.background=\'none\'">' +
          '<button onclick="window._ProfileStudies.generate(\'' + d.id + '\',\'inf\',window.TCI&&window.TCI.cityKey);toggleRapoarteMenu&&toggleRapoarteMenu()" style="flex:1;text-align:left;background:none;border:none;color:' + hex + ';padding:4px 8px;cursor:pointer;border-radius:5px;font-size:11.5px" title="' + (M.inf ? M.inf.t : 'Studiu de amplasament') + ' — necesită parcelă + volum 3D">' + (M.ico || '◆') + ' ' + (d.profile.label || d.id) + '</button>' +
          '</div>';
      }).join('');
    } catch (e) {}
  }

  // ── Meniu TERITORIU: picker pentru studiile TERITORIALE de profil (auto-detectate) ──
  function openTerritorialPicker() {
    try {
      var ck = (G.TCI && G.TCI.cityKey); var city = _resolveCity(ck); var cityName = (city && city.name) || 'UAT';
      var det = (city && G._UATProfile && G._UATProfile.detect) ? G._UATProfile.detect(city) : [];
      det = det.filter(function (d) { return META[d.id] && G._PROFILE_DEEP && G._PROFILE_DEEP[d.id + '_sup']; });
      var old = document.getElementById('profile-terit-modal'); if (old) old.remove();
      var ov = document.createElement('div'); ov.id = 'profile-terit-modal';
      ...
      var items = det.length ? det.map(function (d) {
        var M = META[d.id], ac = M.ac, hex = 'rgb(' + ac[0] + ',' + ac[1] + ',' + ac[2] + ')';
        return '<div style="display:flex;align-items:center;gap:10px;padding:10px 12px;border:1px solid rgba(255,255,255,.08);border-radius:9px;margin-bottom:8px;background:rgba(255,255,255,.02)">' +
          '<div style="flex:1"><div style="color:' + hex + ';font-weight:700;font-size:13px">' + (M.ico || '◆') + ' ' + (d.profile.label || d.id) + '</div>' +
          '<div style="color:#94a3b8;font-size:11px;margin-top:2px">' + (M.sup ? M.sup.t : '') + '</div>' +
          '<div style="color:#64748b;font-size:10px;margin-top:2px">Detectat: ' + d.evidence + ' · încredere ' + d.confidence + '</div></div>' +
          '<button onclick="window._ProfileStudies.generate(\'' + d.id + '\',\'sup\',window.TCI&&window.TCI.cityKey);document.getElementById(\'profile-terit-modal\').remove()" style="flex-shrink:0;background:linear-gradient(180deg,#2563eb,#1d4ed8);color:#fff;border:0;border-radius:8px;padding:9px 13px;cursor:pointer;font-weight:700;font-size:12px">📘 Generează</button></div>';
      }).join('') : '<div style="color:#94a3b8;font-size:13px;padding:20px;text-align:center">Acest UAT (' + cityName + ') nu are un profil teritorial special detectat (litoral / deltă / baraj / minier / salin / portuar / termal / silvic / seismic / transfrontalier).</div>';
      ov.innerHTML = '<div style="max-width:540px;width:100%;background:#0f172a;border:1px solid rgba(56,138,221,.3);border-radius:14px;padding:18px;max-height:85vh;overflow-y:auto;box-shadow:0 16px 50px rgba(0,0,0,.7)">' +
        '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px"><div style="color:#cfe3ff;font-weight:800;font-size:15px">◆ Studii de profil teritorial — ' + cityName + '</div>' +
        '<button onclick="document.getElementById(\'profile-terit-modal\').remove()" style="background:none;border:0;color:#94a3b8;font-size:20px;cursor:pointer">✕</button></div>' +
        '<div style="color:#64748b;font-size:11px;margin-bottom:12px">Profiluri identificate automat din date reale. Generează studiul teritorial / strategic dedicat.</div>' +
        items + '</div>';
      document.body.appendChild(ov);
    } catch (e) { G.ss && G.ss('Eroare picker profil'); }
  }

  G._ProfileStudies = { generate: generate, META: META, renderMenu: renderMenu, openTerritorialPicker: openTerritorialPicker };
  ...
  // ── GATING parcelă+volum pe studiile de amplasament din Rapoarte (cerere Florin) ──
