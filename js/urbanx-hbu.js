...
(function (G) {
  ...
  function _autoChart(D, headers, rows, title) {
    try {
      ...
      var data = rows.map(function (r, i) { var lb = ('' + (r[0] || ('#' + (i + 1)))).replace(/\s+/g, ' ').trim().slice(0, 28); return [lb, vals[i], pal[i % pal.length]]; });
      ...
      if (st === 'donut') D.donut(data.map(function (r) { return [r[0].slice(0, 14), r[1], r[2]]; }), { title: ttl, source: src });
      else if (st === 'pie') D.pie(data.map(function (r) { return [r[0].slice(0, 14), r[1], r[2]]; }), { title: ttl, source: src });
      else if (st === 'hbar') D.hbar(data, { title: ttl, source: src });
      else D.barChart(data.map(function (r) { return [r[0].slice(0, 16), r[1], r[2]]; }), { title: ttl, h: Math.min(58, 26 + data.length * 4), source: src });
    } catch (e) {}
  }
  ...
  async function generatePDF(cityKey, mode) {
    ...
    D.P('Reconversia funcțională a siturilor subutilizate și industriale se desfășoară într-un cadru legislativ și de politici publice complex, a cărui cunoaștere este esențială pentru fezabilitatea proiectului. La nivel național, cadrul de referință este dat de Legea 350/2001 privind amenajarea teritoriului și urbanismul (care reglementează documentațiile de urbanism — PUG, PUZ, PUD — și procedura de schimbare a funcțiunii și a indicatorilor urbanistici), de Legea 50/1991 privind autorizarea executării lucrărilor de construcții, de Codul Civil și de legislația de mediu (OUG 195/2005, legislația privind siturile contaminate). Pentru terenurile cu potențial istoric, Legea 422/2001 privind protejarea monumentelor istorice poate impune avize și restricții suplimentare.');
