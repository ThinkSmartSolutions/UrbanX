...
(function (G) {
  ...
  var IDS = { fill: 'superbloc-fill', perim: 'superbloc-perim', street: 'superbloc-street', plaza: 'superbloc-plaza' };
  function _clear(map) { [IDS.plaza, IDS.street, IDS.perim, IDS.fill].forEach(function (id) { try { if (map.getLayer(id)) map.removeLayer(id); } catch (e) {} try { if (map.getSource(id)) map.removeSource(id); } catch (e) {} }); }
  ...
  function openPanel() {
    ...
    var mapBtn = el('button', { style: ST.ghost + ';margin-top:12px;margin-left:8px' }, '🗺 Înainte/După pe hartă'); body.appendChild(mapBtn);
    ...
    mapBtn.onclick = function () { drawTransform(+side.value || 400, false); ov.remove(); };
    ov.appendChild(m); document.body.appendChild(ov);
  }

  G.Superbloc = { compute: compute, geometry: geometry, cinematicData: cinematicData, drawTransform: drawTransform, drawOverlay: drawOverlay, openPanel: openPanel, chapter: chapter };
