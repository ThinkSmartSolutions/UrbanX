...
(function (G) {
  ...
  function corpuriTbl(D) {
    var c = D.corpuri || []; if (!c.length) return '';
    var sc = 0, sd = 0; c.forEach(function (x) { sc += +x.Sc || 0; sd += +x.Sd || 0; });
    return '<p>Obiectivul cuprinde <b>' + c.length + ' corpuri</b> distincte:</p>' +
      tbl(c.map(function (x, i) { return [esc(x.nume || ('C' + (i + 1))), esc(x.functiune || '—'), esc(x.regim || '—'), n(x.Sc) + ' mp', n(x.Sd) + ' mp']; }).concat([['<b>TOTAL</b>', '', '', '<b>' + n(Math.round(sc)) + ' mp</b>', '<b>' + n(Math.round(sd)) + ' mp</b>']]), ['Corp', 'Funcțiune', 'Regim', 'Sc', 'Sd']);
  }
  ...
  function general(D, v) {
    var ac = (v && v.calc) || {};
    var secs = [];
    secs.push({ h: '1. Date de identificare', html: '<p>Prezenta documentație fundamentează autorizarea obiectivului „' + esc(fnLabel(D)) + '", faza ' + esc(D.faza === 'PTh' ? 'P.Th. — proiect complet de execuție' : 'D.T.A.C. — extras pentru autorizare') + ', amplasat în ' + n(D.uat) + (D.judet ? ', jud. ' + n(D.judet) : '') + (D.nrcad ? ', nr. cad. ' + n(D.nrcad) : '') + '. Beneficiar: ' + n(D.beneficiar) + '. Proiectant: ' + n(D.proiectant) + '.' + (D.nrCU ? ' Certificat de Urbanism nr. ' + n(D.nrCU) + '.' : '') + '</p>' });
    var ct = corpuriTbl(D);
    secs.push({ h: '2. Descrierea investiției', html: '<p>Se propune realizarea unui obiectiv cu funcțiunea „' + esc(fnLabel(D)) + '", regim de înălțime ' + regim(D) + ', suprafață construită ' + n(D.Sc) + ' mp și desfășurată ' + n(D.Sd) + ' mp. Soluția respectă reglementările urbanistice, normativele tehnice specifice destinației și cerințele fundamentale de calitate.</p>' + (ct ? ct : '') + (spatiiTbl(D) ? '<p>Programul funcțional (din modelul confirmat):</p>' + spatiiTbl(D) : '') });
