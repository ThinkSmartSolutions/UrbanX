// cinema-launcher.js — UrbanX Cinema v5.1 FIXED
// FIX #1: cityKey lookup robust — nu mai cade pe Iași dacă UAT-ul e în _UAT_DB sau _EXTRA_UATS
// FIX #2: _PredEngine.calc() nu mai crapa dacă city e undefined — fallback safe
// FIX #3: goScene() nu mai apelează SE._cleanLayers() care strică lightPreset
// FIX #4: cinDraw() — pred.r10 era undefined pentru UAT-uri din _UAT_DB → fallback la 0
// FIX #5: zones sunt recalculate din cx/cy ale UAT-ului selectat (nu hardcodate Iași)
// FIX #6: offsets în scena seismic erau undefined → fix referință
(function(){
'use strict';

var SCENES = [
  {id:'intro',   dur:12000, label:'IDENTITATE'},
...
// ── FIX #1: Lookup robust UAT din toate sursele disponibile ──────────────
function resolveCity(cityKey) {
  var city = null;

  // 1. _RO_CITIES_DB (chei RO-IS-01 style sau siruta match)
  if(typeof window._RO_CITIES_DB !== 'undefined') {
    city = window._RO_CITIES_DB[cityKey];
    if(!city) {
      // Cauta dupa siruta extrasa din key (ex: 'RO-IS-105309' → '105309')
  ...
  // 2. _UAT_DB (chei RO-IS-105309 style)
  if(!city && typeof window._UAT_DB !== 'undefined') {
    city = window._UAT_DB[cityKey];
  }

  // 3. TCI._EXTRA_UATS (comune periurbane)
  if(!city && window.TCI && window.TCI._EXTRA_UATS) {
    city = window.TCI._EXTRA_UATS[cityKey];
  }

  // 4. _UAT_REGISTRY
  if(!city && window._UAT_REGISTRY) {
    var reg = window._UAT_REGISTRY[cityKey];
    if(reg) city = reg;
  }

  // 5. Fallback: primul oras din _RO_CITIES_DB (Iași)
  if(!city && typeof window._RO_CITIES_DB !== 'undefined') {
    var keys = Object.keys(window._RO_CITIES_DB);
    if(keys.length) city = window._RO_CITIES_DB[keys[0]];
  }

  console.log('[Cinema v5.1] resolveCity:', cityKey, '→', city ? city.name : 'NEGASIT');
  return city;
}

// ── FIX #2: _PredEngine.calc() safe — nu mai crapa ──────────────────────
function calcPred(city) {
  if(!city) {
    console.warn('[Cinema v5.1] city undefined — pred fallback');
    return _predFallback({});
  }
  // Incearca _PredEngine
  try {
    if(window._PredEngine && typeof window._PredEngine.calc === 'function') {
      var p = window._PredEngine.calc(city);
      if(p && typeof p === 'object') return _normalizePred(p, city);
    }
  } catch(e) {
    console.warn('[Cinema v5.1] _PredEngine.calc error:', e.message);
  }
  // Incearca TCI._data() ca fallback
  try {
    if(window.TCI && typeof window.TCI._data === 'function') {
      var d = window.TCI._data(2055);
      return _predFallback(city, d);
    }
  } catch(e2) {}
  return _predFallback(city);
}

// Normalizeaza pred — adauga campuri lipsa care cauzau crash in cinDraw
function _normalizePred(p, city) {
  var pop21 = city.pop2021 || city.pop || p.p21 || 100000;
  // FIX #4: r10 poate lipsi daca vine din _UAT_DB
  if(p.r10 === undefined || p.r10 === null) {
    p.r10 = city.rata_reala_2011_2021 || city.rata || -0.5;
  }
  if(!p.p21) p.p21 = pop21;
  if(!p.pop55) p.pop55 = Math.round(pop21 * Math.pow(1 + (p.r10||0)/100, 34));
  if(!p.pib) p.pib = city.pib_capita || 14200;
  if(!p.pctUE) p.pctUE = Math.round((p.pib||14200) / 366 * 10) / 10;
  if(!p.trendLbl) p.trendLbl = (p.r10||0) >= 0 ? 'crestere' : 'scadere';
  if(!p.trendClr) p.trendClr = (p.r10||0) >= 0 ? '#22c55e' : '#ef4444';
  if(!p.defLoc) p.defLoc = Math.max(0, Math.round((pop21 - (p.pop55||pop21)) * 0.4 + pop21 * 0.08));
  if(!p.recHa) p.recHa = Math.round(pop21 / 1000 * 2.8);
  if(!p.ag) p.ag = 0.20;
  if(!p.fond) p.fond = Math.round(pop21 / 50);
  if(!p.mot24) p.mot24 = 380;
  if(!p.satAn) p.satAn = 2038;
  if(!p.fluxOra) p.fluxOra = Math.round(pop21 * 0.08);
  if(!p.pasaje) p.pasaje = Math.max(2, Math.round(pop21 / 40000));
  if(!p.invMob) p.invMob = Math.round(pop21 / 2200);
  if(!p.invTotal) p.invTotal = Math.round(pop21 / 1100);
  if(!p.tp) p.tp = 62;
  if(!p.kmBRT) p.kmBRT = Math.round(pop21 / 7000);
  if(!p.costBRT) p.costBRT = Math.round(p.kmBRT * 3.5);
  if(!p.defTP) p.defTP = Math.max(0, 75 - (p.tp||62));
  if(!p.walkScore) p.walkScore = 58;
  if(!p.statiiNoi) p.statiiNoi = Math.round(pop21 / 2000);
  if(!p.anSUMP) p.anSUMP = 2028;
  if(!p.zile24) p.zile24 = 18;
  if(!p.rRef) p.rRef = (p.r10||0);
  if(!p.anConv) p.anConv = 2050;
  if(!p.pctUE55) p.pctUE55 = Math.min(100, (p.pctUE||39) + 25);
  if(!p.sdgTotal) p.sdgTotal = 6.4;
  if(!p.scoliNoi) p.scoliNoi = Math.max(0, Math.round(pop21 / 80000));
  if(!p.cabMed) p.cabMed = Math.max(1, Math.round(pop21 / 15000));
  if(!p.svHa) p.svHa = Math.round(pop21 / 400);
  if(!p.invSoc) p.invSoc = Math.round(pop21 / 3000);
  if(!p.auth) p.auth = Math.round(pop21 / 800);
  if(p.ocupatie === undefined) p.ocupatie = {servicii: 52, industrie: 28};
  return p;
}

function _predFallback(city, d) {
  var pop21 = (city && (city.pop2021 || city.pop)) || 100000;
  var rata = (city && city.rata_reala_2011_2021) || -0.5;
  return _normalizePred({
    p21: pop21,
    r10: rata,
    pib: (city && city.pib_capita) || 14200,
    pctUE: 39,
    ag: 0.20,
  }, city || {pop2021: pop21});
}
...
window._startCinema = function(cityKey) {
  // Detecteaza cityKey din toate sursele posibile
  cityKey = cityKey
    || (window.TCI && window.TCI.cityKey)
    || (window.S && window.S.activeUAT)
    || (window._ProjectionEngine && window._ProjectionEngine.currentCity)
  ...
  // Suprascrie SCENES si dezactiveaza _setupMap din TCI
  SE.SCENES = SCENES;
  ...
  // FIX #3: NU apelam SE._cleanLayers() — strica lightPreset si face harta neagra
  // Curatam DOAR layerele noastre
  ...
  // FIX #1+#2: resolve city + calc pred safe
  SE._map  = map;
  SE._city = resolveCity(cityKey);
  SE._pred = calcPred(SE._city);
  SE._canvas = cv;
  SE._ctx = cv.getContext('2d');
  SE._playing = true;
  SE._si = 0;

  // Coordonate UAT selectat — NU hardcodate
  var cx   = SE._city.lon || SE._city.centroid?.[0] || 27.601;
  var cy   = SE._city.lat || SE._city.centroid?.[1] || 47.158;
  var pred = SE._pred;
  var name = SE._city.name || 'UAT';

  console.log('[Cinema v5.1] START —', name, '— lon:', cx, 'lat:', cy);
  ...
  // Override flyTo/jumpTo — permite zoom mic DOAR pentru intro si oras3d
  ...
  ctrl.id = 'tci-c8-ctrl';
  ctrl.style.cssText = 'position:fixed;bottom:24px;right:20px;z-index:1000000;display:flex;gap:8px;align-items:center;';
  ctrl.innerHTML = '<button id="c8p" style="background:rgba(0,0,0,.8);border:1px solid rgba(255,255,255,.15);color:#fff;padding:10px 18px;border-radius:10px;cursor:pointer;font:700 13px monospace">◀</button>'
    ...
    +'<div id="c8-name" style="background:rgba(0,0,0,.8);border:1px solid rgba(255,255,255,.10);color:#D4AF37;padding:8px 14px;border-radius:10px;font:700 12px monospace;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+name+'</div>'
  ...
  // Salveaza culoarea originala a cladirilor
  var origColor = null;
  ...
  function stopAll(){
    ...
    if(SE._raf) cancelAnimationFrame(SE._raf);
    if(SE._rotInt){ clearInterval(SE._rotInt); SE._rotInt = null; }
    // FIX #3: curatam DOAR layerele noastre
    try{ if(map.getLayer('cin-osm')) map.removeLayer('cin-osm'); }catch(e){}
    try{ if(map.getSource('cin-osm')) map.removeSource('cin-osm'); }catch(e){}
  ...
  function goScene(i){
    ...
    if(SE._raf) cancelAnimationFrame(SE._raf);
    if(SE._rotInt){ clearInterval(SE._rotInt); SE._rotInt = null; }
    // FIX #3: NU apelam SE._cleanLayers() — curatam DOAR cin-osm
    try{ if(map.getLayer('cin-osm')) map.removeLayer('cin-osm'); }catch(e){}
    try{ if(map.getSource('cin-osm')) map.removeSource('cin-osm'); }catch(e){}
    SE._playing = true;
    runScene(i);
  ...
  document.getElementById('c8pause').onclick = function(){
    if(SE._playing){
      ...
      var loop2 = function(){
        ...
        try{ cinDraw(scene.id, t, SE._ctx, window.innerWidth, window.innerHeight, pred, name, SE); }catch(e){ console.error('[cinDraw]', scene.id, e.message); }
  ...
  function rot(b0, spd){
    ...
    SE._rotInt = setInterval(function(){
      ...
      b += spd;
      try{ map.setBearing(b%360); }catch(e){}
    }, 50);
  }

  // Culorile reale ale cladirilor din OSM
  ...
  // Zone dinamice bazate pe cx/cy UAT-ului selectat
  var zones = [
    {c:[cx,       cy      ], b:20,  z:15.5},
    {c:[cx-0.022, cy+0.015], b:60,  z:15.0},
    {c:[cx+0.025, cy-0.012], b:120, z:15.0},
    {c:[cx-0.012, cy-0.020], b:180, z:15.0},
    {c:[cx+0.018, cy+0.018], b:300, z:15.0},
  ];

  // FIX #6: offsets definite clar (era undefined in scena seismic)
  var offsets = [
    [cx,       cy      ],
    [cx-0.015, cy+0.010],
    [cx+0.018, cy-0.008],
  ...
  function setupScene(id){
    ...
    function smoothDive(zone, preset, startDelay){
      lp(preset||'night');
      var z = zone || zones[0];
      setTimeout(function(){
        if(!SE._playing) return;
        try{ map.jumpTo({center:z.c, zoom:10, pitch:0, bearing:0}); }catch(e){}
        setTimeout(function(){
          if(!SE._playing) return;
          try{ map.flyTo({center:z.c, zoom:12, pitch:20, bearing:z.b*0.2,
    ...
    function glide(zone, delay){
      setTimeout(function(){
        if(!SE._playing) return;
        try{ map.flyTo({center:zone.c, zoom:zone.z, pitch:68, bearing:zone.b,
    ...
    switch(id){
      case 'intro':
        lp('night');
        // Zoom out Romania → regional → oras
        try{ map.jumpTo({center:[24.5,45.8], zoom:6, pitch:0, bearing:0}); }catch(e){}
        setTimeout(function(){ if(!SE._playing)return;
          try{ map.flyTo({center:[(cx+24.5)/2,(cy+45.8)/2], zoom:8, pitch:0, bearing:0,
            duration:4000, essential:true}); }catch(e){} }, 300);
        setTimeout(function(){ if(!SE._playing)return; smoothDive(zones[0],'night',0); }, 4500);
        break;

      case 'oras3d':
        lp('day');
        smoothDive(zones[0],'day',0);
        glide(zones[1], 11000);
        glide(zones[2], 20000);
        break;
      ...
      case 'mobil':
        ...
        fetch('https://urbanx-proxy.3dtravelsoftart.workers.dev/osm?q='+encodeURIComponent(qMob))
          ...
          .then(function(d){
            ...
            console.log('[Cinema] OSM roads:', ft.length, 'pentru', name);
            if(ft.length > 0){
              try{
                if(map.getLayer('cin-osm')) map.removeLayer('cin-osm');
                if(map.getSource('cin-osm')) map.removeSource('cin-osm');
      ...
      case 'seismic':
        ...
        // FIX #6: folosim offsets definite corect
        setTimeout(function(){
          if(!SE._playing) return;
          try{ map.flyTo({center:offsets[1], zoom:14, pitch:65, bearing:30, duration:5000, essential:true}); }catch(e){}
        }, 3000);
        break;
  ...
  function cinDraw(id, t, ctx, W, H, pred, name, SE){
    if(!ctx) return;
    // FIX #2: pred garantat non-null datorita calcPred()
    if(!pred) pred = _predFallback({});
    ...
    switch(id){
      case 'intro':
        ...
        ctx.textAlign = 'center';
        ctx.fillText((SE._city?.judet||SE._city?.judet_code||'—').toUpperCase()+' · '+N(pred.p21)+' LOCUITORI · '+(SE._city?.regiune||''), W/2, H*0.552);
        cifra(N(pred.p21),'Locuitori 2021');
        cifra2(N(Math.round(((SE._city?.suprafata_ha||SE._city?.suprafata||9800)/100)))+' km²','Suprafata');
        break;
      ...
      case 'demo':
        ...
        // FIX #4: r10 garantat non-null
        var r10 = pred.r10 || 0;
        cifra((r10>=0?'+':'')+r10.toFixed(2)+'%/an', pred.trendLbl||'tendinta', pred.trendClr||'#f59e0b');
      ...
      case 'crestere':
        var tGc = t<0.15?0:Math.min(1,(t-0.15)/0.70);
        var tEc = 1-Math.pow(1-tGc,3);
        if(t < 0.18){
          titlu(name+' 2026 — Starea Actuala','Constructii existente'); linie();
          ...
          ctx.globalAlpha = sA*tEc;
          ctx.fillStyle = '#ef4444';
          ctx.font = '900 '+FS.d+'px "Space Grotesk",sans-serif';
          ctx.textAlign = 'left';
          ctx.fillText(N(Math.round((pred.defLoc||5000)*tEc)), W*0.04, H*0.885);
          ...
          if(tEc > 0.5) narativ('Verde=joase/potential densificare. Galben=medii. Rosu=inalte/presiune maxima CC/CP. '+(pred.defLoc||5000)+' unitati necesare.');
          ctx.globalAlpha = 1;
        }
        break;
      ...
      case 'viziune':
        ...
        [{ok:(pred.pop55||0)>(pred.p21||0), txt:'Pop 2055: '+N(pred.pop55)+' loc.'},
  ...
  function runScene(idx){
    ...
    // FIX #3: curatam DOAR layerele noastre — NU cleanLayers global
    try{ if(map.getLayer('cin-osm')) map.removeLayer('cin-osm'); }catch(e){}
    try{ if(map.getSource('cin-osm')) map.removeSource('cin-osm'); }catch(e){}
    ...
    var loop = function(){
      ...
      try{
        cinDraw(scene.id, t, SE._ctx, window.innerWidth, window.innerHeight, pred, name, SE);
      }catch(e){
        console.error('[cinDraw ERROR]', scene.id, e.message, e.stack?.split('\n')[1]);
      }
      if(t < 1){ SE._raf = requestAnimationFrame(loop); }
      else{ runScene(idx+1); }
    };
    SE._raf = requestAnimationFrame(loop);
    console.log('[Cinema v5.1]', idx, scene.id, '—', name);
  }

  runScene(0);
  console.log('[Cinema v5.1] START —', name, '— UAT:', cityKey);
};
...
console.log('[Cinema Launcher v5.1] FIXED — UAT dinamic · cinDraw safe · no cleanLayers crash');
})();
