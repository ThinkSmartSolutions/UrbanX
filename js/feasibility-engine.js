...
(function (G) {
  ...
  function generatePDF(res, meta) {
    ...
    try {
      ...
      D.chapter('19. Analiza fiscală a proiectului');
      D.P('Tratamentul fiscal influențează semnificativ rezultatul net al dezvoltării. Principalele componente fiscale sunt: TVA (deductibil pe achiziții, colectat la vânzare — cu reguli specifice pentru locuințe, unde cota redusă a fost aplicabilă sub anumite plafoane), impozitul pe profit (16% pe profitul societății de proiect), taxele de autorizare și impozitele locale pe teren/clădiri pe durata deținerii. O structurare fiscală eficientă (SPV — societate de proiect dedicată) clarifică fluxurile și limitează riscurile.');
      D.P('Pentru investitorul final, regimul fiscal al venitului (din vânzare sau din chirie) și eventualele facilități pentru clădiri eficiente energetic completează tabloul. Studiul de fezabilitate trebuie să lucreze cu valori nete de impact fiscal pentru a reflecta corect rentabilitatea reală; ipotezele fiscale se verifică cu un consultant de specialitate, întrucât legislația se modifică frecvent.');

      D.chapter('20. Rata de actualizare, inflație și valoarea în timp');
      D.P('Banii au valoare în timp: un profit obținut peste trei ani valorează mai puțin decât același profit azi. Rata de actualizare folosită pentru a aduce fluxurile viitoare la prezent (în calculul VAN) reflectă costul capitalului și prima de risc a proiectului imobiliar — tipic 8–15% pentru dezvoltări rezidențiale, mai mare pentru proiecte speculative. Cu cât riscul perceput e mai mare, cu atât rata e mai mare și valoarea actualizată a profitului mai mică.');
      D.P('Inflația acționează în ambele sensuri: crește prețurile de vânzare (venituri nominale mai mari) dar și costurile de construcție și de finanțare. Într-un mediu inflaționist, contractele cu preț fix de construcție și pre-vânzările la preț ferm protejează marja, în timp ce întârzierile expun proiectul la escaladarea costurilor. Analiza de fezabilitate prudentă testează rezultatul atât în termeni nominali, cât și reali.');

      D.chapter('21. Build-to-sell vs build-to-rent');
      D.P('Modelul de afaceri determină indicatorii relevanți. În build-to-sell (dezvoltare pentru vânzare), accentul cade pe marja de dezvoltare și pe viteza de absorbție — capitalul se recuperează rapid și se reinvestește. În build-to-rent (dezvoltare pentru închiriere), proiectul se evaluează prin randamentul de capitalizare (venit net din chirii / valoarea activului) și prin aprecierea pe termen lung; capitalul rămâne blocat, dar generează venituri recurente și beneficiază de aprecierea imobilului.');
      D.P('Pentru același proiect, cele două modele conduc adesea la decizii diferite de proiectare (mix de unități, dotări, durabilitate a finisajelor) și de finanțare (credit de dezvoltare vs credit ipotecar de investiție). Tot mai mulți dezvoltatori adoptă strategii hibride — vânzarea unei părți pentru recuperarea capitalului și păstrarea altei părți pentru venit recurent. Studiul de fezabilitate trebuie să precizeze clar modelul asumat, întrucât el schimbă fundamental criteriile de succes.');

      D.chapter('22. Bancabilitatea proiectului');
      D.P('Accesul la finanțare bancară depinde de un set de indicatori pe care îi urmărește creditorul. Loan-to-Cost (LTC) și Loan-to-Value (LTV) măsoară gradul de acoperire cu credit raportat la cost, respectiv la valoare — băncile limitează tipic LTC la 60–75%, cerând restul ca aport propriu (skin in the game). Gradul de pre-vânzare (procentul de unități contractate înainte de tragerea creditului de construcție) reduce riscul de absorbție și este adesea o condiție de tragere (40–50% pre-vânzări).');
      D.P('Marja de dezvoltare de ' + rr.margin_pct + '% și randamentul de ' + rr.dev_yield_pct + '% sunt printre primii indicatori evaluați de bancă: o marjă confortabilă oferă o pernă care protejează și creditorul în scenariile adverse. Un proiect bancabil aliniază aceste valori la cerințele creditorului și prezintă un plan de cash-flow credibil, cu garanții și o structură juridică (SPV, ipotecă) clară.');

      D.chapter('23. Sustenabilitate și valoarea verde');
      D.P('Performanța de mediu a devenit un factor economic, nu doar de conformare. Clădirile eficiente energetic (NZEB) și certificate (de exemplu prin scheme de tip BREEAM/LEED/DGNB) beneficiază de un „green premium" — preț și chirie mai mari, absorbție mai rapidă și o valoare mai rezilientă pe termen lung — și de acces la finanțare verde cu cost redus. În același timp, evită riscul de „active blocate" (stranded assets) pe măsură ce reglementarea de mediu se înăsprește.');
      D.P('Integrarea cu modulul Carbon Tracker al platformei permite cuantificarea amprentei de carbon a proiectului, iar coroborarea cu Taxonomia UE deschide accesul la instrumente de finanțare durabilă. Investiția suplimentară în eficiență (anvelopă, surse regenerabile) trebuie evaluată în studiul de fezabilitate nu doar prin costul inițial, ci și prin primele de valoare și prin reducerea riscului de reglementare pe care le aduce.');

      try { if (window.UrbanXIVU && window.UrbanXIVU.renderSection) window.UrbanXIVU.renderSection(D); } catch (e) {}
      D.chapter('24. Limitări și disclaimer');
      ...
      var fn = ('Studiu_Fezabilitate_' + (meta.site_name || 'sit') + '_' + new Date().toISOString().slice(0, 10) + '.pdf').replace(/[ăĂâÂîÎșȘşŞțȚţŢ]/g,function(c){return {'ă':'a','Ă':'A','â':'a','Â':'A','î':'i','Î':'I','ș':'s','Ș':'S','ş':'s','Ş':'S','ț':'t','Ț':'T','ţ':'t','Ţ':'T'}[c]||c;}).replace(/[^a-zA-Z0-9._-]/g,'_');
