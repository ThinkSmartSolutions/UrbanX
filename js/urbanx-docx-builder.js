...
(function (G) {
  ...
  var STYLE = '<style>' +
    ...
    'th{background:#1F3864;color:#fff;border:0.5pt solid #999;padding:4pt;font-size:10pt;text-align:left;word-wrap:break-word;overflow-wrap:break-word}' +
    'td{border:0.5pt solid #999;padding:4pt;font-size:10pt;word-wrap:break-word;overflow-wrap:break-word}' +
  ...
  function _tblVecinatati(vecinatati, neconformM14) {
    ...
    return tbl(vecinatati.map(function (v) {
      ...
      return [(v.id || '—') + (v.cf_numar ? ' (CF ' + v.cf_numar + ')' : ''), v.destinatie_declarata || '—', v.grad_vecin || '—', (v.risc_vecin_estimat || '—').replace('_', ' '),
  ...
  function _tblRezistentaFocElemente(m0, grad, D) {
    ...
    function randRealizatConform(e, necesarMin) {
      ...
      return [realizatTxt, sursaTxt, conform];
    }
    ...
    // Tabel SEPARAT pt trasabilitatea documentelor justificative (Sectiunea 7, audit v6.0) — NU
    // adaugat ca 2 coloane suplimentare la tabelul de mai jos: acela are deja 5 coloane cu text lung
    // (necesar/realizat/sursa), inghesuite intr-o celula de continut de doar 80% din latimea paginii
    // (layout tabelar eticheta|continut) — 7 coloane produceau trunchiere vizibila la tipar/PDF (bug
    // real gasit verificand un document generat, nu doar headless: ultima coloana taiata la marginea
    // paginii). Impartit in 2 tabele cu scop distinct, fiecare incape confortabil.
    var rowsPrincipal = ORDINE.map(function (label) {
      ...
      return [label, n.necesarTxt].concat(randRealizatConform({ cheieProiect: cheieProiect }, n.necesarMin));
    });
    var rowsDocumente = ORDINE.map(function (label) {
      var e = _ELEMENTE_REZISTENTA_FOC.filter(function (x) { return x.label === label; })[0];
      var cheieProiect = e ? e.cheieProiect : cheiProiect[label];
      var docInfo = realizateDeclarate[cheieProiect] && realizateDeclarate[cheieProiect].document_atasat;
      return [label, DOCUMENT_JUSTIFICATIV[label] || 'Fișă tehnică', docInfo ? 'atașat' : 'de_atasat_la_executie'];
    });
    return tbl(rowsPrincipal, ['Element constructiv', 'Rezistență necesară (Tabelul 2)', 'Rezistență realizată (sursă: DoP/certificat)', 'Sursa valorii realizate', 'Conform']) +
      '<p style="font-size:9pt;color:#666;margin-top:6pt"><b>Trasabilitate documente justificative la execuție</b> (Secțiunea 7):</p>' +
      tbl(rowsDocumente, ['Element constructiv', 'Document justificativ necesar', 'Stadiu']);
  }
  ...
  function _tblNeconformitatiV41(fise) {
    ...
    return tbl(fise.map(function (f) {
      return [
        ...
        f.element.identificare_in_plan, f.actiune, (f.cerinta && f.cerinta.sursa_normativa) || '—'
      ];
  ...
  function _buildScenariuSSICascada(D, v) {
    ...
    function fmtSolutiiPtNeconformitate(v14) {
      ...
      return '<p><b>Pentru: ' + cui + cifre + '</b> — soluții candidate (selecția rămâne a proiectantului atestat, nu se aplică automat):</p>' +
        tbl(sol.map(function (x) { return [x.solutie, x.efect_calculat || '—', (x.recalcul_necesar || []).join(', ')]; }), ['Soluție compensatorie', 'Efect', 'Recalcul necesar']);
    }
    ...
    var htmlAtex = atexAplicabilUnele
      ? tbl(rezultateAtex.filter(function (r) { return r.rezultat.ATEX_aplicabil; }).map(function (r) {
        return [r.nume, r.substante, 'Zona ' + r.rezultat.zona_propusa, r.rezultat.echipamente_necesare, 'DE VALIDAT de proiectant ATEX'];
    ...
    var htmlMateriale =
      ...
      (necesitaDoP.length ? tbl(necesitaDoP.map(function (m) {
        ...
        return [m.element || m.nume, m.nume, claseOrientativa, m.DoP_atasat ? 'DA (DoP atașat)' : 'de aprovizionare — produsul concret și DoP se atașează la execuție'];
    ...
    function _detecteazaConflicteNormative() {
      ...
      if (D.are_gaze_naturale === true) {
        conflicte.push({
          ...
          normativ_A: { sursa: 'Ordinul ANRE 89/2018, art. 128/129/141/142', pozitie: 'ventilație naturală permanentă (orificii neetanșate) a spațiului cu consumator de gaz' },
      ...
      return '<p><b>' + conflicte.length + ' conflict/conflicte între normative identificat/identificate</b> — semnalate explicit, fără rezolvare tacită (decizia finală rămâne a proiectanților de specialitate):</p>' +
        tbl(conflicte.map(function (c) {
          return [c.cerinta, c.normativ_A.sursa + ' — ' + c.normativ_A.pozitie, c.normativ_B.sursa + ' — ' + c.normativ_B.pozitie, c.actiune];
    ...
    function _tabelCertitudine() {
      ...
      return '<p>Fiecare valoare-cheie de mai jos poartă un indicator de proveniență, cu procentul de certitudine asociat (0–100%) — nu doar clasificarea vecinătăților, ca în versiunile anterioare ale motorului.</p>' +
        tbl(randuri.map(function (r) { return [r[0], CERTITUDINE_LABEL[r[1]]]; }), ['Valoare', 'Nivel de certitudine']);
    }
    ...
    function _checklistDocumentatieDTAC() {
      ...
      return '<p>' + (nrLipsa ? '<b>' + nrLipsa + ' piesă/piese încă neatașată/neatașate</b> — pieselor scrise/planurilor SSI specifice le rămâne acest stadiu până la încărcarea efectivă în platformă (nu se presupune existența lor).' : 'Toate piesele scrise/planurile specifice SSI urmărite au fost identificate.') + '</p>' +
        tbl(itemi.map(function (i) { return [i.nume, i.prezent ? '✔ prezent' : '✘ lipsă']; }), ['Piesă documentație DTAC/PTh', 'Stare']);
    }
    ...
    function _valideazaLantCauzal() {
      ...
      return '<p>Fiecare verigă e derivată din cea anterioară, nu introdusă independent — unde motorul chiar calculează o verigă din alta (ex. rezerva de apă din debit × durată), se recalculează și se compară explicit; unde legătura programatică nu există încă, se semnalează onest, nu se presupune validă.</p>' +
        tbl(verigi.map(function (v) { return [v.veriga, STARE_LABEL[v.stare] || v.stare, v.obs]; }), ['Verigă a lanțului cauzal', 'Stare', 'Observații']);
    }
    ...
    function _checklistPlanse(D) {
      ...
      return {
        ...
        html: '<p>' + (nrLipsa ? '<b>' + nrLipsa + ' element(e) încă neîncărcate</b> — concluziile care depind de ele rămân marcate onest ca atare în secțiunile corespunzătoare (nu se presupune existența lor).' : 'Toate categoriile de planșe urmărite au cel puțin un fișier încărcat.') + '</p>' +
          tbl(itemi.map(function (i) { return [i.nume, i.prezent ? '✔' : '✘']; }), ['Planșă/Document', 'Stare'])
      };
    }
    ...
    function _checklistCapitole(itemiPlanse) {
      ...
      var html = tbl(capitole.map(function (c) {
        return ['Capitol ' + c.nr + ' (' + c.titlu + ')', c.lipsa.length ? '✘ incomplet' : '✔ complet', c.lipsa.length ? 'lipsește: ' + c.lipsa.map(function (l) { return l.text; }).join(', ') : '—'];
    ...
    // Sectiunea 1 (audit v6.0): motorul de conformitate legislativa — pt FIECARE cerinta aplicabila,
    // un obiect cu schema completa ceruta de audit (cerinta/articol_exact/normativ/status/valoare_ceruta/
    // valoare_proiect/masura_daca_neconform/document_justificativ_necesar/nivel_certitudine). Aceasta e
    // sursa UNICA de adevar — Sectiunea 2 (matricea, pt orice cititor) si Sectiunea 16 (raportul juridic,
    // pt jurist) proiecteaza DIN acest array, nu recalculeaza independent (zero duplicare de logica).
    // articol_exact = 'DE_COMPLETAT — normativ neingerat' cand nu avem inca textul articolului exact
    // extras din normativ (Sectiunea 4 a auditului) — nu se lasa gol, nici generic "conform P118".
    function _obiecteConformitateLegislativa() {
      ...
      var geoDinDWG = !!(D.geometrie_teren && D.geometrie_teren.cladiri_propuse);
      ...
      var obiecte = [
        ...
        { cerinta: 'Numărul de niveluri supraterane', articol_exact: m9niv.norma || 'DE_COMPLETAT — normativ neingerat', normativ: 'P118-1/2025',
          ...
          valoare_ceruta: m9niv.nelimitat ? 'nelimitat pt. gradul ' + grad : (m9niv.niveluri_max != null ? ('max. ' + m9niv.niveluri_max) : null), valoare_proiect: D.niv_supraterane != null ? String(D.niv_supraterane) : null,
          masura_daca_neconform: (!m9niv.nelimitat && m9niv.conform === false) ? 'vezi secțiunea 5' : null, document_justificativ_necesar: null, nivel_certitudine: 'introdus_manual_utilizator' },
        { cerinta: 'Distanțe de siguranță față de vecinătăți', articol_exact: 'Tabelul 4/145', normativ: 'P118-1/2025',
          status: (m6b.vecinatati || []).length ? (vecNeconformeMx ? NECONFORM : CONFORM) : DATE_INSUF,
          valoare_ceruta: 'distanță minimă normată per pereche', valoare_proiect: (m6b.vecinatati || []).length + ' vecinătate/vecinătăți verificate',
          masura_daca_neconform: vecNeconformeMx ? 'vezi secțiunea 5' : null, document_justificativ_necesar: null,
          nivel_certitudine: (m6b.vecinatati || []).some(function (v) { return v.estimat_implicit; }) ? 'presupus_conservator' : (geoDinDWG ? 'extras_automat_DWG' : 'introdus_manual_utilizator') },
        { cerinta: 'Nivelul riscului de incendiu', articol_exact: 'art. A.10.2.1.2/A.10.2.1.3', normativ: 'P118-1/2025',
          status: (riscMx === 'mare' || riscMx === 'foarte mare') ? NECONFORM : CONFORM,
          valoare_ceruta: '≤ 840 MJ/m² (risc mic/mijlociu)', valoare_proiect: 'încadrare: ' + riscMx,
          masura_daca_neconform: (riscMx === 'mare' || riscMx === 'foarte mare') ? 'măsuri suplimentare de protecție conform capitolului 4' : null, document_justificativ_necesar: null,
          nivel_certitudine: camereMx.length ? 'extras_automat_DWG' : 'presupus_conservator' },
        ...
        { cerinta: 'Hidranți de incendiu interiori', articol_exact: 'art. 4.1', normativ: 'P118/2-2013',
          status: ac.hidranti_int_oblig ? DATE_INSUF : NEAPLICABIL, valoare_ceruta: 'peste pragul aplicabil destinației', valoare_proiect: ac.hidranti_int_oblig ? 'obligatoriu, de dimensionat la PTh' : 'sub prag',
          masura_daca_neconform: null, document_justificativ_necesar: ac.hidranti_int_oblig ? 'Fisa tehnica' : null, nivel_certitudine: 'introdus_manual_utilizator' },
        { cerinta: 'Hidranți de incendiu exteriori', articol_exact: 'art. 4.2', normativ: 'P118/2-2013',
          status: ac.hidranti_ext_oblig ? DATE_INSUF : NEAPLICABIL, valoare_ceruta: 'peste pragul aplicabil destinației', valoare_proiect: ac.hidranti_ext_oblig ? 'obligatoriu, de dimensionat la PTh' : 'sub prag',
          masura_daca_neconform: null, document_justificativ_necesar: ac.hidranti_ext_oblig ? 'Fisa tehnica' : null, nivel_certitudine: 'introdus_manual_utilizator' },
        ...
        { cerinta: 'Instalație electrică cu rol SSI (iluminat de siguranță, DDR)', articol_exact: 'art. 7.23.8.1 (iluminat), art. 4.2.2.8 (DDR), art. 7.22 (alimentare rezervă)', normativ: 'I7-2011 (amendat Ord. MDLPA 959/2023)',
          status: DATE_INSUF, valoare_ceruta: 'DDR obligatoriu (curent stabilit după caracteristicile instalației, nu prag fix din varianta pre-2023); iluminat siguranță dacă >50 pers./Sc>300mp/subteran>100mp', valoare_proiect: 'de detaliat la proiectul electric', masura_daca_neconform: null, document_justificativ_necesar: 'Certificat', nivel_certitudine: 'extras_automat_normativ_nevalidat' },
        // I 20-2000, art. 2.2.1 (cazuri de echipare OBLIGATORIE, extras pe text oficial 13 iul) —
        // ac.paratraznet_oblig acum chiar calculat (bug real: era mereu undefined inainte, vezi
        // urbanx-doc-engine.js). Pt cazurile NEacoperite de criteriile automate (sali aglomerate/
        // spitale/hoteluri cu capacitate mare, monumente istorice), ramane onest "date insuficiente"
        // — nivelul de protectie (I-IV) rezulta din calculul Nd/Nc (art. 2.1.10, Tabelul 8), nu e
        // presupus.
        { cerinta: 'Protecție împotriva trăsnetului (IPT/SPT)', articol_exact: ac.paratraznet_oblig ? 'art. 2.2.1 (cazuri obligatorii)' : 'art. 2.1.10 (calcul Nd/Nc)', normativ: 'I 20-2000',
          status: DATE_INSUF, valoare_ceruta: ac.paratraznet_oblig ? 'obligatorie (categorie pericol incendiu A/B, clădire înaltă H>28m, sau rezidențial >P+11E)' : null,
          valoare_proiect: ac.paratraznet_oblig ? 'necesară — nivelul de protecție (I-IV) rezultă din calculul Nd/Nc, art. 2.1.10' : 'de evaluat prin calculul Nd/Nc (art. 2.1.10) la proiectul electric',
          masura_daca_neconform: null, document_justificativ_necesar: ac.paratraznet_oblig ? 'Fisa tehnica' : null, nivel_certitudine: 'extras_automat_normativ_nevalidat' },
        // Corectie normativ (13 iul, cercetare text oficial): "NP 127" e gresit — acel indicativ e
        // normativul de securitate la incendiu a PARCAJELOR subterane, nu al instalatiilor de gaze.
        // Normativul real, in vigoare: Ordinul ANRE nr. 89/2018 (M.Of. 462/2018), amendat de Ordinul
        // ANRE nr. 2/2023 — art. 129 alin. (5) e articolul care leaga direct instalatia de gaze de SSI
        // (detectorul automat de gaze, daca exista, se conecteaza obligatoriu la centrala de semnalizare
        // a incendiului cand cladirea are IDSAI) — extras pe text oficial (149 pag., 412 articole).
        { cerinta: 'Instalație de gaze naturale', articol_exact: D.are_gaze_naturale === true ? 'art. 129 alin. (5)' : 'art. 128/129/141/142 (ventilatie), art. 66/1421 (electrovalve)', normativ: 'Ordinul ANRE nr. 89/2018 (amendat 2/2023)',
        ...
        { cerinta: 'Sursă electrică de rezervă (generator)', articol_exact: 'art. 7.22/7.22.1 (pornire ≤15s, preluare ≤60s), art. 5.6.3.1 (tip sursă)', normativ: 'I7-2011 (amendat Ord. MDLPA 959/2023)',
          status: (ac.sprinklere_oblig || ac.hidranti_int_oblig || ac.idsi_oblig || ac.desfumare_oblig) ? DATE_INSUF : NEAPLICABIL, valoare_ceruta: (ac.sprinklere_oblig || ac.hidranti_int_oblig || ac.idsi_oblig || ac.desfumare_oblig) ? 'pornire automată ≤15s, preluare eșalonată ≤60s' : null, valoare_proiect: 'de dimensionat la PTh',
      ...
      if (cladiriPropuse.length > 1 && m5bGrupuri) {
        ...
        obiecte.splice(1, 0, { cerinta: 'Compartimentarea grupurilor constructive alipite (ansamblu)', articol_exact: 'Tabelul 146/147', normativ: 'P118-1/2025',
          status: grupNeconform ? NECONFORM : CONFORM, valoare_ceruta: null, valoare_proiect: m5bGrupuri.length + ' grup(uri) verificat(e)',
          masura_daca_neconform: grupNeconform ? 'vezi secțiunea 5' : null, document_justificativ_necesar: null, nivel_certitudine: 'extras_automat_DWG' });
      }
      if (cladiriPropuse.length > 1 && m6c) {
        obiecte.splice(cladiriPropuse.length > 1 && m5bGrupuri ? 5 : 4, 0, { cerinta: 'Distanțe între clădirile proprii ale ansamblului', articol_exact: 'Tabelul 4/145', normativ: 'P118-1/2025',
          status: m6c.neconforme.length ? NECONFORM : CONFORM, valoare_ceruta: null, valoare_proiect: m6c.perechi.length + ' pereche/perechi verificată/verificate',
          masura_daca_neconform: m6c.neconforme.length ? 'vezi secțiunea 5' : null, document_justificativ_necesar: null, nivel_certitudine: 'extras_automat_DWG' });
      }
    ...
    // Sectiunea 1 (audit v6.0) — randare tehnica detaliata, schema completa per cerinta (audienta:
    // proiectant/verificator ISU care vrea TOT — status enum, valori exacte, masura, document, certitudine).
    function _tblMotorConformitateDetaliat() {
      var obiecte = _obiecteConformitateLegislativa();
      // NU esc() aici — tbl()/_cell() escapeaza deja fiecare celula o singura data; un esc() suplimentar
      // aici produce dublu-escapare (bug real gasit intr-un document generat: "Sc&gt;3.000" afisat
      // LITERAL in loc de "Sc>3.000" — valorile cu caractere reale >/</& din obiectele de conformitate
      // (praguri Sc>3000, H>28m) treceau prin esc() de doua ori). Coloanele Status/Cerință nu au nevoie
      // de citat separat, sunt deja text simplu. Impartit in 2 tabele (nu 9 coloane intr-unul singur —
      // acelasi bug de trunchiere gasit la tabelul de rezistenta la foc, vezi mai sus) pt lizibilitate.
      // 3 tabele a cate 3-4 coloane (nu 1 singur de 9, nici 2 de 6) — un tabel de 6 coloane cu text
      // lung (articol_exact/valori) tot trunchia ultima coloana la marginea paginii (gasit vizual la
      // randare PDF reala, nu doar headless) — fiecare tabel de mai jos incape confortabil.
      return '<p>Pentru fiecare cerință aplicabilă, motorul emite un verdict explicit cu schema completă (nu doar un calcul izolat) — sursa unică din care derivă atât matricea de sinteză (mai jos), cât și raportul juridic (anexă).</p>' +
        tbl(obiecte.map(function (o) {
          return [o.cerinta, o.articol_exact, o.normativ, o.status];
        }), ['Cerință', 'Articol exact', 'Normativ', 'Status']) +
        '<p style="font-size:9pt;color:#666;margin-top:6pt"><b>Valoarea cerută vs. valoarea din proiect, per cerință:</b></p>' +
        tbl(obiecte.map(function (o) {
          return [o.cerinta, o.valoare_ceruta || '—', o.valoare_proiect || '—'];
        }), ['Cerință', 'Valoare cerută', 'Valoare proiect']) +
        '<p style="font-size:9pt;color:#666;margin-top:6pt"><b>Măsură (dacă neconform), document justificativ și nivel de certitudine per cerință:</b></p>' +
        tbl(obiecte.map(function (o) {
          return [o.cerinta, o.masura_daca_neconform || '—', o.document_justificativ_necesar || '—', CERTITUDINE_LABEL[o.nivel_certitudine] || o.nivel_certitudine];
        }), ['Cerință', 'Măsură dacă neconform', 'Document justificativ', 'Nivel de certitudine']);
    }

    // Sectiunea 2 (audit v6.0): matricea completa de conformitate — capitol final obligatoriu, PROIECTAT
    // din obiectele Sectiunii 1 (nu recalculat independent) — randurile aplicabile difera cu functiunea/
    // tipul de lucrare, dar formatul tabelului e identic pt orice proiect.
    function _tblMatriceConformitate() {
      var STATUS_DISPLAY = { CONFORM: 'DA', NECONFORM: 'NU', NEAPLICABIL: 'NEAPLICABIL', DATE_INSUFICIENTE: 'DATE INSUFICIENTE' };
      var obiecte = _obiecteConformitateLegislativa();
      // Impartit in 2 tabele (5 coloane cu "Observatii" lung trunchia vizibil la randare PDF reala,
      // acelasi gen de bug ca la tabelele de mai sus) — Cerinta ramane ancora comuna intre ele.
      return '<p>Tabel generat automat din obiectele de conformitate ale Secțiunii 1 (nu o listă fixă scrisă manual per funcțiune) — rândurile aplicabile depind de destinația și tipul de lucrare ale acestui proiect.</p>' +
        tbl(obiecte.map(function (o) { return [o.cerinta, o.articol_exact, o.normativ, STATUS_DISPLAY[o.status] || o.status]; }), ['Cerință legală', 'Articol/Tabel', 'Normativ', 'Conform']) +
        '<p style="font-size:9pt;color:#666;margin-top:6pt"><b>Observații per cerință:</b></p>' +
        tbl(obiecte.map(function (o) {
          var obs = [o.valoare_proiect, o.masura_daca_neconform].filter(Boolean).join(' — ') || '—';
          return [o.cerinta, obs];
        }), ['Cerință legală', 'Observații']);
    }
    ...
    function _tblRaportJuridic() {
      ...
      // NU esc() — tbl()/_cell() escapeaza deja o singura data (vezi nota din _tblMotorConformitateDetaliat).
      return '<p>Tabel distinct de matricea tehnică de mai sus — orientat strict juridic, pentru un jurist/verificator care nu are nevoie de calcul, ci doar de starea de conformitate legală per act normativ.</p>' +
        tbl(randuri.map(function (r) {
          var row = ['—', '—', '—', '—']; row[COL[r.status]] = '✔ ' + r.obs;
          return [r.normativ].concat(row);
    ...
    var secs = [
      ...
      { h: '1.1. Datele de identificare', html: tbl([
        ['Denumire', D.nume || '—'], ['Amplasament', (D.uat || '—') + (D.nrcad ? ', nr. cad. ' + D.nrcad : '')],
        ['Beneficiar', D.beneficiar || '—'], ['Fază de proiectare', D.faza || 'D.T.A.C.'],
        ['Telefon/E-mail beneficiar', D.contact_beneficiar || '[se completează]']
      ], ['Element', 'Valoare']) },
      ...
      html: '<p>Amprentele la sol au fost extrase automat din planul de situație (DXF) — fiecare clădire distinctă e o compartimentare separată de incendiu, dacă nu sunt alăturate/interconectate fără separare la foc. Suprafețele (Sc/Sd) și indicatorii (POT/CUT) de mai jos provin din adnotările proiectantului din desen, nu sunt recalculate.</p>' +
        tbl(urbanismAnsamblu.tipuri.map(function (t) { return [t.denumire, '' + t.n, t.sc_mp + ' m²', t.sd_mp != null ? t.sd_mp + ' m²' : '—']; }), ['Tip clădire', 'Nr. unități', 'Sc/unitate', 'Sd/unitate']) +
        tbl([
          ...
          ['Suprafață teren (' + (urbanismAnsamblu.sursaArieTeren || 'sursă nedeterminată') + ')', urbanismAnsamblu.arieTeren_mp ? urbanismAnsamblu.arieTeren_mp.toLocaleString('ro-RO') + ' m²' : 'nedeterminată (mapează layerul LIMITĂ DE PROPRIETATE sau adaugă adnotări „S.parcelă" în plan)'],
      ...
      html: '<p>Fiecare compartiment din 1.4.f de mai sus provine dintr-o componentă conexă geometrică (clădiri ale căror contururi sunt practic alipite, &lt; 0,3 m) — tabelul arată DIN CE clădiri e compus și de ce (arie/volum/conform sunt deja la 1.4.f, nu se repetă aici).</p>' +
        tbl(m5bGrupuri.map(function (g) {
          return [g.id_grup, g.tip.replace(/_/g, ' '), g.cladiri_incluse.join(', '), (g.tratament || '').replace(/_/g, ' ')];
      ...
      { h: '2.2.a\'. Categoria de pericol de incendiu (A-E)', html: '<p>Categoria implicită asociată funcțiunii (' + esc(destinatieT42.toLowerCase()) + '): <b>' + esc(categoriePericolIncendiuDefault) + '</b> — valoare de pornire per funcțiune (js/urbanx-doc-engine.js).</p>' +
        (m0.regim_tabele === 'EXISTENTA_NEMODIFICATA'
          ? '<p>Pentru construcții <b>EXISTENTE</b>, literele A-E sunt definite explicit în <b>Anexa A.10, Art. A.10.2.1.4-A.10.2.1.5 și Tabelul 143</b> (verificat verbatim 19 iul 2026), pe baza naturii substanțelor/materialelor din spațiu: A (BE3a) — lichide cu temp. de inflamabilitate a vaporilor ≤28°C sau gaze/vapori cu LIE ≤10% ce pot forma amestecuri explozive cu aerul; B (BE3b) — lichide 28-100°C sau gaze/vapori cu LIE &gt;10%, sau fibre/praf/pulberi în cantități ce pot forma amestecuri explozive; C (BE2) — substanțe/materiale combustibile solide, sau lichide &gt;100°C (risc devine mare/foarte mare dacă densitatea sarcinii termice &gt;105 MJ/m²); D (BE1a) — substanțe incombustibile fierbinți/topite/incandescente sau foc deschis, fără substanțe combustibile; E (BE1b) — substanțe incombustibile reci sau materiale combustibile cu umiditate &gt;80% (aprindere exclusă).</p>'
          : '<p><b>Notă importantă:</b> pentru construcții <b>NOI</b>, P118-1/2025 (corpul principal) <b>nu mai folosește literele A-E</b> ca sistem de clasificare — acestea sunt specifice Anexei A.10 (construcții existente). Clasificarea operativă pentru acest proiect este <b>riscul de incendiu pe 4 niveluri</b> (mic/mijlociu/mare/foarte mare), stabilit din densitatea sarcinii termice conform <b>Art. 2.1.2.2 și Tabelul 132 (Anexa 8.1)</b> — vezi secțiunea 2.1 de mai sus pentru calculul efectiv pe acest proiect. Categoria A-E afișată aici este o <b>corespondență convențională</b> păstrată pentru compatibilitate cu alte normative care încă o referă direct (ex. I 20-2000, pentru protecția la trăsnet), nu o cerință proprie P118-1/2025 pentru construcții noi.</p>') +
        (categoriePericolIncendiuNota ? '<p><b>Categorie finală recalculată: ' + esc(categoriePericolIncendiuFinal) + '</b> — ' + esc(categoriePericolIncendiuNota) + ' Temei recalculare: zonare ATEX conform <b>H.G. 1058/2006, art. 9</b> (prezența/frecvența atmosferei explozive determină zona 0/20 sau 1/21), corelat cu pragul de echipare obligatorie IPT pt. categoria A/B (<b>I 20-2000, art. 2.2.1 lit. i)</b>) și cu aria maximă de compartiment recalculată (' + (ac.arie_compartiment_max || 0).toLocaleString('ro-RO') + ' mp).</p>' : '<p>Nicio zonă ATEX periculoasă (0/20/1/21) nu a fost identificată din substanțele declarate — categoria rămâne cea implicită a funcțiunii.</p>') },
      ...
      { h: '3.6.c. Ascensoare de pompieri', html: (function () {
        // FIX BUG REAL (18 iul — dupa recitirea scenariului real): textul vechi spunea "Nu este cazul"
        // NECONDITIONAT, indiferent de D.H — o cladire inalta reala (H>28m) ar fi primit aceeasi
        // concluzie falsa. Acum verifica H-ul real al proiectului.
        var H = +D.H || 0;
        if (H <= 28) return '<p>Nu este cazul — înălțimea reală a construcției (' + (H || 'necompletată, regim ' + esc(D.regim || 'P+1E')) + (H ? ' m' : '') + ') nu atinge pragul care impune ascensor de intervenție (cerință specifică clădirilor înalte/foarte înalte, H≥28/45m).</p>';
        return '<p>Construcția (H=' + H + ' m) este <b>clădire înaltă' + (H > 45 ? '/foarte înaltă' : '') + '</b> — necesită <b>ascensor de intervenție (pompieri)</b>, conform P118-1/2025: tip — ascensor cu funcționare specială în regim de incendiu (comandă prioritară pompieri); număr — minimum 1 per casă de scări care deservește nivelurile peste 28 m; caracteristici — cabină minimum 630 kg/8 persoane, timp de sosire la nivelul cel mai înalt sub 60 secunde; amplasare — accesibil din incinta tampon/casa scării protejate, cu acces direct din exterior la nivelul de bază; sursă de alimentare electrică de rezervă — conform 4.16; timp de siguranță — minimum egal cu timpul necesar evacuării/intervenției la nivelul cel mai defavorabil. Caracteristicile exacte (număr de ascensoare, capacitate, viteză) se stabilesc la faza de proiect tehnic, funcție de configurația reală a caselor de scări.</p>';
      })() },
      { h: null, html: (function () {
      ...
      { h: '4.10. Instalație electrică cu rol în securitatea la incendiu', html: (function () {
        // I7-2011, amendat Ordinul MDLPA 959/2023 — extras pe text oficial (13 iul). Iluminat de
        // siguranta evacuare: pct. 7.23.8.1, prag REAL (nu presupunere) — obligatoriu daca >50 pers.
        // simultan SAU Sc>300 mp SAU subteran>100 mp. DDR: pct. 4.2.2.8 — corectie fata de varianta
        // anterioara a acestui motor (spunea gresit "≤300mA" ca prag fix): textul AMENDAT 2023 NU mai
        // are un prag numeric fix, curentul nominal se stabileste dupa caracteristicile instalatiei;
        // ramane insa OBLIGATORIU (nu doar recomandat) pt. scoli/sanatate/comert/turism/cladiri lemn/
        // cluburi-discoteci/cladiri inalte-f.inalte/monumente istorice.
        var capacitateDecl = D.capacitate_persoane || D.capacitate_declarata;
        var areSubteranMare = (+D.niv_subterane || 0) > 0 && (+D.Sc || 0) > 100;
        var necesitaIluminatSiguranta = (capacitateDecl && capacitateDecl > 50) || (+D.Sc || 0) > 300 || areSubteranMare;
      ...
      { h: '4.11. Instalație de protecție împotriva trăsnetului', html: '<p>Necesitatea echipării cu instalație de protecție împotriva trăsnetului (IPT/SPT) se stabilește conform <b>I 20-2000, art. 2.2.1</b> — normativul prevede echipare <b>obligatorie</b>, indiferent de calculul de risc, pentru o listă explicită de cazuri (art. 2.2.1 lit. a-n): construcții cu categorie de pericol de incendiu A sau B, clădiri înalte/foarte înalte (H&gt;28 m, conform P118), clădiri de locuit colective cu regim peste P+11E, săli aglomerate (&gt;400 locuri), spitale (&gt;75 paturi), hoteluri/cămine (&gt;400 paturi), școli (&gt;10 săli de clasă), monumente istorice, coșuri/turnuri/silozuri ≥10 m (cu înălțime ≥2× înălțimea construcțiilor învecinate), stadioane (&gt;5.000 locuri). În afara acestor cazuri, necesitatea se stabilește prin calculul <b>Nd vs. Nc</b> (art. 2.1.10): dacă frecvența lovirilor directe așteptate (Nd) depășește frecvența acceptată (Nc, funcție de tip construcție/conținut/grad ocupare/consecințe — art. 2.1.9, tabelele 4-7), IPT devine necesară. Concluzie pentru acest proiect: <b>' + (ac.paratraznet_oblig ? 'NECESARĂ (cade sub unul din cazurile obligatorii ale art. 2.2.1)' : 'de evaluat prin calculul Nd/Nc') + '</b> — pentru destinația și regimul de înălțime ale acestui proiect (' + esc(destinatieT42.toLowerCase()) + ', ' + esc(D.regim || '—') + '). Nivelul de protecție (I-IV, corespunzător eficacității E calculate) rezultă din Tabelul 8 (art. 2.1.10), iar metoda de captare (tijă/conductoare întinse/plasă) din art. 2.3.11-2.3.13 — ambele rămân responsabilitatea proiectantului de instalații electrice la faza de proiect tehnic; nu se presupune implicit nici necesară, nici inexistentă.</p>' +
      ...
      { h: '4.12. Instalație de gaze naturale (Ordinul ANRE nr. 89/2018, amendat 2/2023)', html: '<p>' + (D.are_gaze_naturale === true
        ? 'Proiectul declară branșament/instalație de gaze naturale — se aplică Ordinul ANRE nr. 89/2018 (nu „NP 127", indicativ care desemnează în realitate normativul de securitate la incendiu a parcajelor subterane). Cerințe reale aplicabile: <b>ventilația spațiului cu consumatori de gaze</b> — volum minim al încăperii și ventilare naturală/mecanică conform I5-2010 (art. 128), suprafață vitrată minimă 0,03–0,05 m²/m³ pentru geamul de decomprimare (art. 129), gol de acces aer de ardere S=0,0025×Qi la partea inferioară (art. 135), ventilare naturală permanentă a subsolului prin orificii pe conturul exterior (art. 141), orificiu Ø150–200 mm la partea superioară a casei scării (art. 142); <b>interacțiunea cu SSI</b> — dacă geamul de decomprimare are grosime &gt;4 mm sau e securizat/Thermopan, este obligatoriu un detector automat de gaze cu electrovalvă de închidere automată (art. 129 alin. (2)), iar dacă proiectul are IDSAI, detectorul de gaze se conectează obligatoriu la centrala de semnalizare a incendiului, inclusiv semnalarea defectului/întreruperii alimentării (art. 129 alin. (5)) — acesta este articolul-cheie de coordonare cu scenariul SSI.'
        : (D.are_gaze_naturale === false ? 'Nu este cazul — proiectul nu declară branșament/instalație de gaze naturale.' : '<b>Nedeclarat pentru acest proiect</b> (D.are_gaze_naturale) — de precizat de proiectant la faza de proiect tehnic; nu se presupune nici prezența, nici absența unei instalații de gaze.')) + '</p>' },
      { h: '4.13. Presurizare case de scări', html: '<p>' + (ac.presurizare_scari_oblig
        ? 'Necesară — configurația/regimul de înălțime al proiectului impune presurizarea casei de scări ca alternativă/completare la evacuarea naturală a fumului (se dimensionează la faza de proiect tehnic).'
        : 'Nu este cazul — regimul de înălțime redus (' + esc(D.regim || '—') + ') permite evacuarea fumului prin mijloace naturale (vezi 3.4.a), fără presurizare mecanică a casei de scări.') + '</p>' },
      { h: '4.14. Ventilație mecanică generală — interacțiune cu instalația de securitate la incendiu', html: '<p>' + (D.are_ventilatie_mecanica === true
        ? 'Proiectul declară ventilație mecanică generală — aceasta trebuie oprită automat la detectarea incendiului (comandă de la centrala de semnalizare, conform P118-3/2015), pentru a nu alimenta focul cu aer proaspăt sau a propaga fumul între compartimente prin tubulatură comună.'
        : (D.are_ventilatie_mecanica === false ? 'Nu este cazul — proiectul nu declară ventilație mecanică generală.' : '<b>Nedeclarat pentru acest proiect</b> (D.are_ventilatie_mecanica) — de precizat de proiectant la faza de proiect tehnic; dacă se adaugă ulterior o instalație de ventilație mecanică, interacțiunea cu SSI (oprire automată la incendiu) devine obligatorie.')) + '</p>' },
      { h: '4.15. Surse de apă pentru stingerea incendiilor (rezervă/bazin de apă)', html: '<p>' + ((ac.hidranti_ext_oblig || ac.sprinklere_oblig)
        ? 'Necesară — hidranții exteriori și/sau instalația de sprinklere obligatorii pentru acest proiect (vezi 4.2/4.3) necesită o rezervă de apă pentru incendiu, estimată la <b>' + (ac.rezerva_incendiu_mc || 0).toLocaleString('ro-RO') + ' m³</b> (P118/2-2013 — hidranți interiori 4,2 l/s×10min + hidranți exteriori 10 l/s×3h, după caz), dimensionată exact la faza de proiect tehnic; rezervor propriu sau racord la rețeaua publică cu debit/presiune garantate, de confirmat prin aviz de la operatorul de apă.'
        : 'Nu este cazul — nici hidranții exteriori, nici instalația de sprinklere nu sunt obligatorii pentru acest proiect (vezi 4.2/4.3), deci nu se impune o rezervă de apă dedicată incendiului.') + '</p>' },
      { h: '4.16. Sursă electrică de rezervă (generator)', html: '<p>' + ((ac.sprinklere_oblig || ac.hidranti_int_oblig || ac.idsi_oblig || ac.desfumare_oblig)
        ? 'Necesară — cel puțin o instalație activă de securitate la incendiu este obligatorie pentru acest proiect (hidranți/sprinklere/IDSAI/desfumare, vezi secțiunile de mai sus), iar funcționarea acesteia pe durata unui incendiu (inclusiv la întreruperea alimentării de bază) necesită o sursă de rezervă (generator electric sau alt mijloc echivalent), dimensionată la faza de proiect tehnic.'
        : 'Nu este cazul — niciuna dintre instalațiile active de securitate la incendiu (hidranți/sprinklere/IDSAI/desfumare) nu este obligatorie pentru acest proiect, deci nu se impune o sursă electrică de rezervă dedicată SSI.') + '</p>' },
      { h: '4.17. Alimentare electrică de rezervă a căilor de evacuare', html: '<p>Iluminatul de siguranță pentru evacuare (vezi 4.10) trebuie alimentat dintr-o sursă independentă de alimentarea de bază (acumulatori proprii ai corpurilor de iluminat, sau circuit dedicat pe sursa de rezervă de la 4.16), astfel încât să rămână funcțional și la întreruperea alimentării generale, pe durata minimă normată de evacuare — se detaliază la proiectul de instalații electrice.</p>' },
      ...
      { h: 'Motorul de conformitate legislativă — detaliu complet per cerință', html: _tblMotorConformitateDetaliat() },
  ...
  var DOC_BUILDERS = {
    ...
    'Scoatere teren din circuitul agricol (Ord. 83/2018)': function (D, v) {
      ...
      var secs = [
        ...
        { h: '4. Încadrarea în clase de calitate și taxa', html: '<p>Taxa de scoatere din circuitul agricol se calculează prin înmulțirea suprafeței (mp) cu tariful UNIC corespunzător clasei de calitate/bonitate a solului (I-V), prevăzut în Anexa nr. 1 la Legea 18/1991 (modificată prin Legea 231/2018) și indexat anual prin ordin al ministrului agriculturii — <b>NU</b> în funcție de categoria de folosință (arabil/vie/livadă etc.), care rămâne doar o dată descriptivă din studiul pedologic. Terenurile de clasă superioară (I-II) au taxa cea mai mare. Valoarea exactă a clasei se stabilește pe baza studiului pedologic (OSPA); valoarea tarifului în vigoare se verifică la data depunerii (Monitorul Oficial).</p>' + tbl([['Clasa I', 'taxă maximă'], ['Clasa II-III', 'taxă medie'], ['Clasa IV-V', 'taxă minimă']], ['Clasă de calitate', 'Nivel taxă']) },
        { h: '5. Documente necesare + avize', html: '<p>Documentație cadastrală, extras CF, CU, studiu pedologic (OSPA), aviz APM (după caz), plan de amplasament. Pentru extravilan: aviz DADR/APIA. Actul de scoatere se emite de autoritatea competentă (APIA/MADR/consiliul județean, funcție de suprafață), conform procedurii Ord. MADR 83/2018.</p>' }
      ];
      return { cat: 'Avize', file: 'Scoatere_circuit_agricol.doc', html: docHtml(_meta(D, 'SCOATERE DIN CIRCUITUL AGRICOL', 'Legea 18/1991 (Anexa 1, mod. Legea 231/2018) · procedură Ord. MADR 83/2018'), secs) };
    },
    ...
    'Fișe tehnice dotări și materiale': function (D, v) {
      ...
      var htmlDotari = dotari
        ? tbl(dotari.map(function (e) { return [e.denumire || '—', e.tip || '—', e.caracteristici || '—', e.certificare || 'de precizat']; }), ['Denumire', 'Tip/categorie', 'Caracteristici tehnice', 'Certificare/agrement'])
